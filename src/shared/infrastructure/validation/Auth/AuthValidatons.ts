...
export class AuthValidations {
    ...
    readonly registerValidation = [
        body('email', 'El correo es requerido').notEmpty().trim().isEmail().withMessage('El correo no es valido'),
        body('password', 'La contraseña es requerida').trim().isLength({ min: 8 }).withMessage('La contraseña debe contener al menos 8 caracteres'),
        body('fullname', 'El nombre es requerido').trim().isLength({ min: 8 }).withMessage('El nombre debe contener al menos 8 caracteres'),
        // body('phone.phone_number', 'El telefono es requerido').trim().isLength({ min: 8 }).withMessage('El nombre debe contener al menos 8 caracteres'),
        // body('phone.prefix', 'El prefijo es requerido').notEmpty().trim(),
