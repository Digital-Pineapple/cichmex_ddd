...
import { ProductEntity, ResponseProductsByCategory } from "./ProductEntity"
...
export interface ProductRepository extends MongoRepository {
    ...
    findProductsByCategory(categoryId: ObjectId, storehouse: string, queryparams: Object): Promise<ResponseProductsByCategory[] | ErrorHandler | null>
    GetProductPaginate(skip: number, limit: number): Promise<ProductEntity[] | null>
    GetOutOfStock(skip: number, limit: number, minNumber: number, page: number): Promise<any | null>
