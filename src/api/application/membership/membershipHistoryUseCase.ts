...
export class MembershipHistoryUseCase {
 ...
 public async createOneHistoryMembership( membershipBenefit_id:string ): Promise<MembershipHistory> {
    ...
    public async getSalesDay(date_service:any, branch_office_id:any): Promise<MembershipHistory[] | ErrorHandler | null> {
        return await this.membershipHistoryRepository.getSalesDayByBranch(date_service,branch_office_id)
    }
    
    public async consumeBenefit(id:any, membershipBenefit_id:any, date_service:string, typeCar_id:any, car_color:string, plate_number:string, branch_office_id:any ): Promise<MembershipHistory | ErrorHandler> {
